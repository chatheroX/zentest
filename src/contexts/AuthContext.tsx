...
import type { AuthenticatedUser, AdminTableType, UserTableType, LicenseKeyTableType } from '@/types/supabase'; // Updated types
...
// --- Constants for Auth ---
const SESSION_COOKIE_NAME = 'proctorchecker-session'; // Renamed cookie
...
const DEFAULT_DASHBOARD_ROUTE = USER_DASHBOARD_ROUTE; // Default for general users
...
function SupabaseConfigErrorDisplay({ errorMessage }: { errorMessage: string }) {
  // Same styling as before, this component remains unchanged internally
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', backgroundColor: '#F0F4F8', color: '#1E293B', padding: '20px', fontFamily: 'sans-serif', textAlign: 'center', boxSizing: 'border-box' }}>
      <div style={{ backgroundColor: '#FFFFFF', padding: '30px 40px', borderRadius: '16px', boxShadow: '0px 10px 20px rgba(30, 41, 59, 0.1), 0px 4px 8px rgba(30, 41, 59, 0.05)', maxWidth: '600px', width: '100%' }}>
        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: '0 auto 20px auto' }}><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
        <h1 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#DC2626', marginBottom: '15px' }}>Application Configuration Error</h1>
        <p style={{ fontSize: '1rem', color: '#334155', marginBottom: '10px' }}>The application cannot start correctly because a critical configuration for backend services is missing or invalid.</p>
        <p style={{ fontSize: '1rem', fontWeight: '600', color: '#1E293B', marginBottom: '20px' }}><strong>Error Detail:</strong> {errorMessage}</p>
        <div style={{ backgroundColor: '#F1F5F9', padding: '15px', borderRadius: '8px', textAlign: 'left', marginBottom: '25px', border: '1px solid #E2E8F0' }}>
          ...
          <p style={{ fontSize: '0.95rem', color: '#475569', marginBottom: '8px' }}>1. Locate or create a <strong><code>.env</code></strong> file in your project root.</p>
          <p style={{ fontSize: '0.95rem', color: '#475569', marginBottom: '8px' }}>2. Ensure the variable <strong><code>{varName}</code></strong> is correctly set (e.g., <code>{varName}=&quot;{exampleValue}&quot;</code>).</p>
          <p style={{ fontSize: '0.95rem', color: '#475569' }}>3. Save the <code>.env</code> file and restart your development server.</p>
        </div>
        <p style={{ fontSize: '0.875rem', color: '#64748B' }}>Contact support if the issue persists after correction.</p>
      </div>
    </div>
  );
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<AuthenticatedUser | null>(null); // Generic user type
  ...
  // Placeholder Dicebear avatar generation
  const generateAvatar = (id: string, role: 'user' | 'admin'): string => {
    const style = role === 'admin' ? 'shapes' : 'identicon'; // Different styles for admin/user
    return `https://api.dicebear.com/8.x/${style}/svg?seed=${encodeURIComponent(id)}&backgroundColor=2196f3,4caf50,ff9800,e91e63&backgroundType=gradientLinear&shapeColor=ffffff,f44336,00bcd4`;
  };

  // Step 1: Initial critical config check
  useEffect(() => {
    ...
    if (currentConfigError) {
      console.error(`[AuthContext Init] ${currentConfigError}`);
  ...
  useEffect(() => {
    if (!initialConfigCheckDone || configError || supabase) return;
    try {
      const client = createSupabaseBrowserClient();
      setSupabase(client);
      console.log("[AuthContext Init] Supabase client initialized.");
    } catch (e: any) {
      const errorMsg = getSafeErrorMessage(e, "Failed to initialize Supabase client.");
      console.error(`[AuthContext Init] ${errorMsg}`, e);
      setConfigError(errorMsg); setAuthError(errorMsg); setIsLoading(false);
    }
  }, [initialConfigCheckDone, configError, supabase]);

  // Step 3: Load user from cookie
  const loadUserFromCookie = useCallback(async () => {
    if (!initialConfigCheckDone || configError || !supabase) {
        if (initialConfigCheckDone && !configError && !supabase && !authError) setAuthError("Service connection not ready.");
        setIsLoading(false);
        return;
    }
    ...
    try {
      ...
      const loadedUser: AuthenticatedUser = {
        ...
        role: roleCookie,
        avatar_url: generateAvatar(userDataFromDb.id, roleCookie),
  ...
  const performSignIn = async (username: string, pass: string, type: 'user' | 'admin'): Promise<{ success: boolean; error?: string; user?: AuthenticatedUser | null }> => {
    ...
    try {
      ...
      // !! IMPORTANT: Plain text password comparison. REPLACE with hash comparison in production !!
      if (data.password_hash === pass) { // In production, this should be bcrypt.compare(pass, data.password_hash)
        const authUser: AuthenticatedUser = {
          id: data.id,
          username: data.username,
          role: type,
          avatar_url: generateAvatar(data.id, type),
          saved_links: type === 'user' ? (data as UserTableType['Row']).saved_links : null,
        };
        setUser(authUser);
        Cookies.set(SESSION_COOKIE_NAME, authUser.username, { expires: 7, path: '/' });
        Cookies.set(ROLE_COOKIE_NAME, authUser.role, { expires: 7, path: '/' });
  ...
  const registerUserWithLicense = async (licenseKey: string, username: string, pass: string): Promise<{ success: boolean; error?: string; user?: AuthenticatedUser | null }> => {
    ...
    try {
      ...
      // 3. Create user (password should be hashed here in a real app)
      const newUserRecord: UserTableType['Insert'] = { username, password_hash: pass, license_key_used_id: keyData.id, saved_links: [] };
      ...
      if (updateKeyError) { /* Log this, but proceed with login if user created */ console.warn("Failed to mark license as claimed:", updateKeyError); }

      const authUser: AuthenticatedUser = {
        id: insertedUser.id, username: insertedUser.username, role: 'user',
        avatar_url: generateAvatar(insertedUser.id, 'user'), saved_links: insertedUser.saved_links || [],
      };
      setUser(authUser);
      Cookies.set(SESSION_COOKIE_NAME, authUser.username, { expires: 7, path: '/' });
      Cookies.set(ROLE_COOKIE_NAME, authUser.role, { expires: 7, path: '/' });
  ...
  }), [user, isLoading, authError, supabase, signInUser, signInAdmin, registerUserWithLicense, updateUserLinks, showSignOutDialog]);

  if (!initialConfigCheckDone && !configError) {
    return ( <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}><p>Initializing Application...</p></div> );
  }
  if (configError) {
    ...
    <AuthContext.Provider value={contextValue}>
      ...
      <AlertDialog open={showSignOutDialog} onOpenChange={setShowSignOutDialog}>
        <AlertDialogContent className="glass-pane">
          ...
          <AlertDialogFooter><AlertDialogCancel className="btn-outline" onClick={() => setShowSignOutDialog(false)}>Cancel</AlertDialogCancel><AlertDialogAction onClick={performSignOut} className="btn-gradient-destructive">Logout</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
